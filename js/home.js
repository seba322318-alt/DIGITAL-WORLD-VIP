...
if(s.hero_video_enabled!==false && selectedHeroVideo){source.src=selectedHeroVideo;video.poster=heroPoster;video.load();video.classList.remove('hidden');video.addEventListener('loadeddata',()=>posterEl.classList.add('video-ready'),{once:true});video.addEventListener('error',()=>{video.classList.add('hidden');posterEl.classList.remove('video-ready')},{once:true})}else{video.classList.add('hidden')}
...
if(configured){const [{data:p},{data:c}]=await Promise.all([supabase.from('site_pages').select('*').in('slug',['home-marketing','viajes','estilo-vida']),supabase.from('site_cards').select('*').eq('page_slug','home-marketing').eq('active',true).order('sort_order')]);for(const row of p||[])pages[row.slug]=row;if(c?.length)marketing=c}
const marketingPage=pages['home-marketing']||fallbackPages['home-marketing'];
document.querySelector('#marketingEyebrow').textContent=marketingPage.eyebrow||'Ecosistema Digital World VIP';document.querySelector('#marketingTitle').textContent=marketingPage.title||'Conocimientos pensados para el mundo digital';document.querySelector('#marketingSubtitle').textContent=marketingPage.subtitle||'';document.querySelector('#marketingCards').innerHTML=marketing.map((c,i)=>`<article class="marketing-card ${i===0?'marketing-card-large':''}"><img src="${escapeHtml(c.image_url||'')}" alt="${escapeHtml(c.title||'Marketing digital')}" loading="lazy"><div class="marketing-overlay"><span class="marketing-number">${escapeHtml(c.badge||String(i+1).padStart(2,'0'))}</span><h3>${escapeHtml(c.title||'')}</h3><p>${escapeHtml(c.body||'')}</p></div></article>`).join('');
function applyPromo(slug,prefix){const p=pages[slug]||fallbackPages[slug];const card=document.querySelector(`#${prefix}Promo`);const nav=document.querySelector(`#${prefix}Nav`);if(p.active===false){card?.classList.add('hidden');nav?.classList.add('hidden');return}document.querySelector(`#${prefix}PromoTitle`).textContent=p.title||'';document.querySelector(`#${prefix}PromoText`).textContent=p.subtitle||'';const bg=document.querySelector(`#${prefix}PromoBg`);if(bg)bg.style.backgroundImage=`url("${String(p.hero_image_url||'').replace(/["\\]/g,'')}")`}
applyPromo('viajes','travel');applyPromo('estilo-vida','lifestyle');
if(configured){const [{data:offers},{data:trading}]=await Promise.all([supabase.from('digital_offers').select('*').eq('active',true).order('sort_order').limit(4),supabase.from('trading_settings').select('*').eq('id',1).maybeSingle()]);const preview=document.querySelector('#digitalOffersPreview');if(preview){preview.innerHTML=(offers||[]).map((o,i)=>`<a class="offer-preview-card" href="/ofertas.html"><div class="offer-preview-media">${o.image_url?`<img src="${escapeHtml(o.image_url)}" alt="${escapeHtml(o.name)}" loading="lazy">`:`<div class="offer-placeholder"><span>${escapeHtml((o.name||'DW').slice(0,2).toUpperCase())}</span></div>`}</div><div class="offer-preview-body"><span>${String(i+1).padStart(2,'0')}</span><h3>${escapeHtml(o.name)}</h3><p>${escapeHtml(o.subtitle||'Acceso digital disponible')}</p><div class="mini-prices"><b>${localMoney(o.price_local||0)}</b><b>${money(o.price_usd||0,'USD')}</b></div></div></a>`).join('')||'<div class="notice">Próximamente encontrarás aquí accesos digitales.</div>'}if(trading){const card=document.querySelector('#tradingPromoHome');if(trading.active===false)card?.classList.add('hidden');document.querySelector('#tradingHomeEyebrow').textContent=trading.eyebrow||'NUEVA ERA';document.querySelector('#tradingHomeTitle').textContent=trading.title||'TRADING — LA ERA DIGITAL';document.querySelector('#tradingHomeText').textContent=trading.subtitle||'';if(trading.cover_url)document.querySelector('#tradingHomeBg').style.backgroundImage=`url("${String(trading.cover_url).replace(/["\\]/g,'')}")`}}