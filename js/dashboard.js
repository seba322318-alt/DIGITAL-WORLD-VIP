    ...
    for(const plan of allowedPlans){
      ...
      for(const c of planCourses){
        ...
        for(const l of lessons){
          ...
          renderLessonVideo(el.querySelector('.lesson-video-area'),l);
          await renderResources(
            el.querySelector('.lesson-resources-area'),
            (l.lesson_resources||[]).filter(r=>r.active).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0))
          );
...
function renderLessonVideo(host,l){
  ...
  if(yid){
    ...
    wrap.innerHTML=`
      <iframe
        src="https://www.youtube-nocookie.com/embed/${encodeURIComponent(yid)}?controls=1&rel=0&modestbranding=1&playsinline=1&iv_load_policy=3"
        title="${escapeHtml(l.title||'Video de la clase')}"
        allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture; fullscreen"
        allowfullscreen
        loading="lazy"
        referrerpolicy="strict-origin-when-cross-origin"></iframe>
      <div class="youtube-top-click-shield" aria-hidden="true"></div>`;

    const shield=wrap.querySelector('.youtube-top-click-shield');
...
async function renderResources(host,resources){
  ...
  for(const r of resources){
    if(r.resource_type==='image'&&r.storage_path){
      ...
      a.className='resource-image-card';
    ...
    if(r.resource_type==='link'&&r.external_url){
      const a=document.createElement('a');
      a.className='resource-file-card';
      a.href=r.external_url;
      a.target='_blank';
      a.rel='noopener noreferrer';
      a.innerHTML=`<span class="resource-icon">↗</span><div><b>${escapeHtml(r.title||'Enlace')}</b><small>Abrir recurso</small></div>`;
      grid.appendChild(a);
      continue;
    }

    if(r.storage_path){
      const {data,error}=await supabase.storage.from('academy-files').createSignedUrl(
        r.storage_path,
        3600,
        {download:r.original_name||true}
      );
      if(error) continue;

      const a=document.createElement('a');
      a.className=`resource-file-card resource-${r.resource_type}`;
      a.href=data.signedUrl;
      a.target='_blank';
      a.rel='noopener';
